...
public class ValidarArquivo {
	@Test
	public void verificaArquivoComparando() {
		...
		try (InputStream pdfStream = pdf.asInputStream(); PDDocument document = PDDocument.load(pdfStream)) {
	...
	@Test
	public void verificaConteudoPdf() throws IOException {
		...
		Response retorno = RestAssured.given()
			...
			try (InputStream pdfStream = retorno.asInputStream(); PDDocument document = PDDocument.load(pdfStream)) {
	...
	@Test
	public void validaArquivoConvertidoEmByte() throws IOException {

		Response retorno = RestAssured.given().when().get("/beneficiario/3821/mensalidade/5395/fatura/csv").then()
